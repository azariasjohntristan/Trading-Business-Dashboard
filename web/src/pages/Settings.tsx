  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground">User and system settings</p>
      </div>

      <section className="space-y-3">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">User Settings</h2>
      ...
      <section className="space-y-3">
        ...
        <div className="rounded-lg border bg-card p-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={toggleTheme}
              className={cn('relative h-6 w-11 rounded-full transition-colors', dark ? 'bg-primary' : 'bg-muted')}
            >
      ...
      <section className="space-y-3">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Timezone</h2>
        <div className="rounded-lg border bg-card p-4">
      ...
      <section className="space-y-3">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">System Status</h2>
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-lg border bg-card p-4">
            <p className="text-[11px] text-muted-foreground">Backend</p>
            <p className="text-sm font-bold text-success">Running</p>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <p className="text-[11px] text-muted-foreground">Database</p>
            <p className={cn('text-sm font-bold', health?.database === 'connected' ? 'text-success' : 'text-destructive')}>
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            {lastImport ? (
              <div>
                <p className="text-xs font-medium">{lastImport.filename}</p>
                <p className="text-[11px] text-muted-foreground">{new Date(lastImport.importedAt).toLocaleString()}</p>
              </div>
            ) : (
